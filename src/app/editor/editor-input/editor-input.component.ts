  ...
  providers: [
    {
      ...
      useExisting: EditorInputComponent,
      multi: true
    }
  ]
})
export class EditorInputComponent
  ...
  constructor(
    ...
    protected controller: EditorControllerService,
    protected htmlParser: EditorHtmlParserService,
    protected parser: EditorParserService,
    protected sanitizer: DomSanitizer,
    protected factoryResolver: ComponentFactoryResolver,
    protected legacyBrowser: EditorLegacyBrowserService
  ) {}
  @Input()
  public uiId = 'default';
  ...
  public ngAfterViewInit() {
    this.editTarget = this.el.nativeElement.querySelector('.editor__edit--target');
    if (this.row) {
      this.computeHeight();
    }
    ...
    this.editTarget.addEventListener('keydown', (event: any) => {
      ...
      if (
        ...
        if (
          ...
          node.nodeName === '#text' &&
          !this.legacyBrowser.findParentElement(node).isContentEditable
        ) {
          // this is the case on webkit
          // console.debug('deleting inside un-editable block detected');
          this.selectionRange.selectNode(
            this.legacyBrowser.findParentElement(node)
          );
          this.selectionRange.deleteContents();
          stopDefaultAndForceUpdate();
        }
    ...
    this.uiSubscription = this.controller.channel(uiId).subscribe(command => {
      ...
      if (command.name === 'insertHTML' && this.selectionRange) {
        ...
        this.selectionRange.insertNode(
          document.createRange().createContextualFragment(command.value)
        );
  ...
  protected selectionInEditableTarget() {
    ...
    return (
      ...
      (node === this.editTarget ||
        this.legacyBrowser.findParent(node, '.editor__edit--target') ===
          this.editTarget)
    );
  }
